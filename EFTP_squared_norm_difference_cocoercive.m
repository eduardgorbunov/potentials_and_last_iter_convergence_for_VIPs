...
for N = 1:Nmax
    ...
    objective = (bargk(:,end))'*G*(bargk(:,end)) - (bargk(:,end-2))'*G*(bargk(:,end-2)); % this is \|F(x^N)\|^2 - \|F(x^{N-1})\|^2

    % this is for the constraints on F
    for i = 2:nbPts
        for j = 1:i
            if i~=j & ((i ~= 3) | (i == 3 & j == 1)) 
                constraint = constraint + ( L*(barg(:,i) - barg(:,j))'*G*(barx(:,i) - barx(:,j)) >= (barg(:,i) - barg(:,j))'*G*(barg(:,i) - barg(:,j)));
                %  L*<g^i - g^j, x^i - x^j> >= ||g^i - g^j||^2
            end
        end
    end
    %
    ...
    save(strcat('dump/EFTP_cocoercive_norm_diff_L_1_N_', sprintf('%d_', N), sprintf('_%f', gamma),'.mat'), 'res_norm', 'gamma');
