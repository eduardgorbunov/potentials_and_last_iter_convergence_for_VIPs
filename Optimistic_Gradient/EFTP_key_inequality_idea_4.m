...
for N = Nmax:Nmax
    ...
    objective = 0.1*(N+2)*(bargk(:,end))'*G*(bargk(:,end)) - 0.1*(N+1)*(bargk(:,end-2))'*G*(bargk(:,end-2)) - 2*(bargk(:,end-2))'*G*(bargk(:,end)) + (bargk(:,end))'*G*(bargk(:,end));
    ...
    save(strcat('dump/EFTP_key_ineq_idea_4_dist_2_res_L_1_N_', sprintf('%d_', N), sprintf('_%f', gamma),'.mat'), 'res_norm', 'gamma');
    ...
    save(strcat('dump/EFTP_key_ineq_idea_4_dist_2_dual_variables_L_', sprintf('%d_', L), sprintf('N_%d_', N), sprintf('_%f', gamma),'.mat'), 'res_norm', 'gamma', 'monotonicity_weights', 'Lipschitzness_weights');
    %fprintf("======================================================\n");
end