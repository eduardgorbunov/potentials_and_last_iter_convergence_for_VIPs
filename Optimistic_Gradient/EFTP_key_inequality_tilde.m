...
for N = 1:Nmax
    ...
    objective = (bargk(:,2))'*G*(bargk(:,2)); % this will be \|F(x^N)\|^2 - (sum that we know how to estimate)
    for i = 2:N
        objective = objective - 2*(bargk(:,2*i-2))'*G*(bargk(:,2*i)) + (bargk(:,2*i))'*G*(bargk(:,2*i));
    end

    objective = (N+1)* (bargk(:,end-1))'*G*(bargk(:,end-1)) + objective * 10.0;
    ...
    save(strcat('dump/EFTP_key_ineq_tilde_dist_2_res_L_1_N_', sprintf('%d_', N), sprintf('_%f', gamma),'.mat'), 'res_norm', 'gamma');
    ...
    save(strcat('dump/EFTP_key_ineq_tilde_dist_2_dual_variables_L_', sprintf('%d_', L), sprintf('N_%d_', N), sprintf('_%f', gamma),'.mat'), 'res_norm', 'gamma', 'monotonicity_weights', 'Lipschitzness_weights');
    %fprintf("======================================================\n");
end