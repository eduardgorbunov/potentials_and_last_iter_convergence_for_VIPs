...
for N = Nmax:Nmax
    ...
    objective = (bargk(:,end-1))'*G*(bargk(:,end-1)); % this is \|F(tx^N)\|^2

    % this is for the constraints on F
    for i = 2:nbPts
        for j = 1:i
            if i~=j & ((i ~= 3) | (i == 3 & j == 1)) & (i - j <= 2 | j == 1)
                if (j ~= 1) & ((mod(i,2) ~= 1) | (j ~= i-1)) & ((mod(i,2) ~= 0) | (j ~= i-2)) & ((mod(i,2) ~= 1) | (j ~= i-2) | (i >= nbPts-8) | (i <= 9))
                    constraint = constraint + ( (barg(:,i) - barg(:,j))'*G*(barg(:,i) - barg(:,j)) - L^2 * (barx(:,i) - barx(:,j))'*G*(barx(:,i) - barx(:,j)) <= 0);
                %  \|g^i - g^j\|^2 \leq L^2*\|x^i - x^j\|
                end
                if ((j ~= 1) | (mod(i,2) ~= 0)) & (((mod(i,2) == 1) & (j == 1)) | ((mod(i,2) == 0) & (j == i-2) & (j ~= 1)) )%| ((i == 4) & (j == 1)))
    ...
    save(strcat('dump/EFTP_tilde_norm_L_', sprintf('%d_', L), sprintf('N_%d_', N), sprintf('_%f', gamma),'.mat'), 'res_norm', 'gamma');
    ...
    save(strcat('dump/EFTP_tilde_dual_variables_L_', sprintf('%d_', L), sprintf('N_%d_', N), sprintf('_%f', gamma),'.mat'), 'res_norm', 'gamma', 'monotonicity_weights', 'Lipschitzness_weights');
    fprintf("======================================================\n");
end