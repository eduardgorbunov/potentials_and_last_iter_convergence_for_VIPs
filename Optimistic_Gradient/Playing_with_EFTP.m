...
%   hx^{k+1} = x^{k+1} + gamma_k g^k                (final iterate of EFTP)
%
%
%   we study worst-case behavior of \|F(hx^N)\|^2 
%   under the condition that ||x_0 - x_*|| is bounded.
%
%
%   Let's PEP-it!
%   - P = [ x0 g0 g1 ... gN hgN]   and G = P^T P
%
...
for N = 2:Nmax
    ...
    dimG    = N + 3;        % dimension of G
    nbPts   = N + 3; % this is x_*, x_0, ..., x_N, hx_N
    ...
    bargk = zeros(dimG, N+2); bargk(2:end,:) = eye(N+2); % so that bargk(:,i) returns gradient of x_{i-1}
    barxk = zeros(dimG, N+2); barxk(:,1) = barx0;

    barxk(:,2) = barxk(:,1) - gamma * bargk(:,1);
    for i = 2:N
        barxk(:,i+1) = barxk(:,i) - 2*gamma * bargk(:,i) + gamma * bargk(:,i-1);
    end
    barxk(:,N+2) = barxk(:,N+1) + gamma * bargk(:,N);
    ...
    objective = (bargk(:,end))'*G*(bargk(:,end)); % this is \|F(hx^N)\|^2
    
    % this is for the constraints on F
    for i = 2:nbPts
        for j = 1:i
            if i~=j %& (i - j <= 2 | j == 1 | j == 2 | i == nbPts | i == nbPts - 1)
    ...
    save(strcat('dump/EFTP_norm_L_1_N_', sprintf('%d_', N), sprintf('_%f', gamma),'.mat'), 'res_norm', 'gamma');
