...
Nmax = 2; %CODE WORKS ONLY FOR N = 2
...
for N = Nmax:Nmax
    ...
    barxk(:,4) = barxk(:,3) - gamma * bargk(:,2);
    barxk(:,5) = barxk(:,3) - gamma * bargk(:,4);
    ...
    objective = (bargk(:,end))'*G*(bargk(:,end)) - (bargk(:,end-2))'*G*(bargk(:,end-2));
    ...
    save(strcat('dump/EFTP_squared_norm_difference_res_L_1_N_', sprintf('%d_', N), sprintf('_%f', gamma),'.mat'), 'res_norm', 'gamma');
    ...
    save(strcat('dump/EFTP_squared_norm_difference_dual_variables_L_', sprintf('%d_', L), sprintf('N_%d_', N), sprintf('_%f', gamma),'.mat'), 'res_norm', 'gamma', 'monotonicity_weights', 'Lipschitzness_weights');
    %fprintf("======================================================\n");
end