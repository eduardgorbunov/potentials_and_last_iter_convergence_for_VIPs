...
%                 constraint = constraint + ( (barg(:,i) - barg(:,j))'*G*(barx(:,i) - barx(:,j)) >= 0);
                ...
                if ((mod(i,2) == 1) & (j == 1)) | ((mod(i,2) == 0) & (j == i-2) & (j ~= 1)) | ((i == 4) & (j == 1))
                    ...
                    %if ((mod(i,2) == 1) & (j == 1) & (i > 3) & (i < nbPts-3))
                    %    constraint = constraint + (lambda_monotone(i,j)==lambda_monotone(i-2,j));
                    %end
                    if ((mod(i,2) == 0) & (j == i-2) & (j ~= 1) & (i >= 8))
                        constraint = constraint + (lambda_monotone(i,j)-lambda_monotone(i-2,j-2) == lambda_monotone(i-2,j-2)-lambda_monotone(i-4,j-4));
                    end
                else
    ...
    save(strcat('dump/EFTP_2_simplified_dual_8_3_norm_L_', sprintf('%d_N_', L), sprintf('%d_', N), sprintf('_%f', gamma),'.mat'), 'res_norm', 'gamma');
...
%                 constraint = constraint + ( (barg(:,i) - barg(:,j))'*G*(barx(:,i) - barx(:,j)) >= 0);
    ...
    %    end
    %end
    %save(strcat('dump/EFTP_2_simplified_dual_8_3_dual_var_L_', sprintf('%d_', L), sprintf('N_%d_', N), sprintf('_%f', gamma),'.mat'), 'res_norm', 'gamma', 'monotonicity_weights', 'Lipschitzness_weights');
