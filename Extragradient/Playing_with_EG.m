...
%
%   tx^{k+1} = x^k - \gamma_k g^k  with g^k = F(x^k)
...
for N = 1:Nmax
    ...
    barxk = zeros(dimG, 2*N+1); barxk(:,1) = barx0;
    
    for i = 1:N
        barxk(:,2*i) = barxk(:,2*i-1) - gamma * bargk(:,2*i-1);
    ...
    save(strcat('dump/EG_norm_L_1_N_', sprintf('%d_', N), sprintf('_%f', gamma),'.mat'), 'res_norm', 'gamma');
